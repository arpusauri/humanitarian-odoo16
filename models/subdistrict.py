...
class Subdistrict(models.Model):
    ...
    kec = fields.Many2one(comodel_name='humanitarian.district', string='Nama Kecamatan', domain="[('city', '=', kota)]", required=False)
    kdkota = fields.Char(string="Kode Kota", size=16, required=False)
    kota = fields.Many2one(comodel_name='humanitarian.city', string='Nama Kota', domain="[('province', '=', provinsi)]", required=False)
    provinsi_id = fields.Many2one(comodel_name='humanitarian.province', string='Nama Provinsi', required=False)
    kdprov = fields.Char(string="Kode Provinsi", size=16, required=False)
     
    @api.onchange('district')
    def _onchange_kdkec(self):
        self.kdkec = self.district.kdkec
